...
- The firmware applies light M5Unified mic conditioning before PCM12 encoding:
  ...
  volume override below 0 dB, and a 5-tap FIR low-pass while downsampling from
  16 kHz to 8 kHz. The stream intentionally avoids nonlinear speech processing
  and soft limiting because recent WAVs showed headroom but audible distortion.
  These values are also exposed in the BLE device-info JSON for debugging
  recordings.
